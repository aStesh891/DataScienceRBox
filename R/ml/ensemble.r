...
# Код посвящен улучшению производительности моделей машинного обучения с использованием
# методов оптимизации параметров, баггинга, бустинга и случайных лесов.

##### --------------------------------------

# load the credit dataset (credit.csv)
...
m <- train(default ~ ., data = credit, #Автоматический подбор модели C5.0 на основе входных данных
           method = "C5.0")
...
#Настраивает процесс кросс-валидации для 10 фолдов.
...
#RNGversion("3.5.2") # use an older random number generator to match the book
set.seed(123)
mybag <- bagging(default ~ ., data = credit, 
                 nbagg = 25)
...
library(C50)
(m_c50_bst <- C5.0(default ~ ., data = credit, 
                   trials = 100))
...
#RNGversion("3.5.2") # use an older random number generator to match the book
set.seed(123)
adaboost_cv <- boosting.cv(default ~ ., 
                           data = credit)
adaboost_cv$confusion
chisq.test(adaboost_cv$confusion)
# calculate kappa
library(vcd)
...
# mtry: Количество случайных предикторов для разделения в дереве.
...
#RNGversion("3.5.2") # use an older random number generator to match the book
set.seed(123)
m_rf <- train(default ~ ., data = credit,
              method = "rf",
              metric = "ROC", trControl = ctrl, #Оптимизируется AUC ROC.
              tuneGrid = grid_rf)
m_rf
...
plot(roc_rf, col = "darkred", legacy.axes = TRUE)
plot(roc_c50, col = "darkblue", add = TRUE)
